        ...
        Entity exploder = null;
            if (explosionParameters.inWater) {
                if (!MCH_Config.DefaultExplosionParticle.prmBool) {
                    ...
                    List<BlockPos> affectedPositions = explosionParameters.getAffectedPositions();
                    MCH_ExplosionV2.effectVanillaExplosion(player.world, explosionParameters.x, explosionParameters.y, explosionParameters.z, explosionParameters.size, affectedPositions);
                }
            } else {
                MCH_ExplosionV2.effectExplosionInWater(player.world, explosionParameters.x, explosionParameters.y, explosionParameters.z, explosionParameters.size);
            }
    }
